...
typedef struct _IMAGE_IMPORT_DESCRIPTOR {
    ...
    DWORD Name;                          // RVA to name of imported DLL
    DWORD FirstThunk;                    // RVA to IAT (if bound this points to \* IMAGE_THUNK_DATA)
...
// Updated function declarations using PE Context
int validate_pe_structure(FILE *f, unsigned char **buffer, long *file_size);
int initialize_pe_context(const char *input_path, PE_Context *ctx);
void cleanup_pe_context(PE_Context *ctx);
int find_executable_sections(PE_Context *ctx, PIMAGE_SECTION_HEADER **valid_sections, size_t *num_valid_sections);
int detect_overlaps_and_calculate_size(PIMAGE_SECTION_HEADER *valid_sections,
                                       size_t num_valid_sections, size_t *total_shellcode_size);
int is_section_included(PIMAGE_SECTION_HEADER section);
int parse_section_name(const char *section_str, char ***section_names, size_t *count);
...
// Helper function to safely copy section names
void safe_copy_section_name(const BYTE *src, char *dst, size_t dst_size);

// Helper function for safe integer addition with overflow checking
int safe_add_size(size_t a, size_t b, size_t *result);

// Initialize PE Context from input file
int initialize_pe_context(const char *input_path, PE_Context *ctx) {
    ...
    if (validate_pe_structure(f, &ctx->buffer, &ctx->file_size) != 0) {
        return 1;  // validate_pe_structure already freed resources and printed errors
    }
...
// Comparison function for sorting sections by raw data address
...
// Helper function to convert RVA (Relative Virtual Address) to file offset
// This eliminates ~15 duplicated loops throughout the codebase
DWORD rva_to_file_offset(DWORD rva, PIMAGE_SECTION_HEADER section_table,
                          WORD num_sections, long file_size) {
    if (rva == 0) return 0;
...
// Helper function to safely copy section names with bounds checking
void safe_copy_section_name(const BYTE *src, char *dst, size_t dst_size) {
...
// Find executable sections in the PE file
int find_executable_sections(PE_Context *ctx, PIMAGE_SECTION_HEADER **valid_sections,
    ...
    } else if (characteristics & 0x0002) {  // IMAGE_FILE_EXECUTABLE_IMAGE
    ...
    *num_valid_sections = 0;
    ...
    if (verbose && ctx->entry_point_rva != 0) {
        for (size_t i = 0; i < *num_valid_sections; i++) {
            ...
            if (ctx->entry_point_rva >= section_start && ctx->entry_point_rva < section_end) {
